...
class PortfolioPlan2024CoverageRepairRequest:
    repo_root: Path
    ...
    def __post_init__(self) -> None:
        ...
        if self.target_year != 2024:
            raise ValueError("Portfolio Plan 2024 repair is locked to target_year=2024")
        if self.focus_start_dt != FOCUS_START or self.focus_end_dt != FOCUS_END:
            raise ValueError(
                "Portfolio Plan 2024 repair focus window is fixed to 2024-01-02..2024-01-05"
            )
...
@dataclass(frozen=True)
class PortfolioPlan2024CoverageRepairSummary:
    run_id: str
    status: str
