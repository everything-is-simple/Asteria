...
def portfolio_plan_report_dir(report_root: Path, run_id: str) -> Path:
    return report_root / "portfolio_plan" / utc_now().date().isoformat() / run_id
...
def summary_status(
    *,
    ...
    if hard_fail_count != 0:
        return "failed"
    if followup_next_card != TRADE_REPAIR_CARD:
        return "failed"
    if followup_attribution != "downstream_surface_gap:trade":
        return "failed"
    return "completed"
