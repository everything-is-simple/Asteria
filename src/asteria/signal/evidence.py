...
from asteria.signal.contracts import SignalBuildSummary
...
def write_signal_evidence(
    ...
    report_dir = report_root / "signal" / _utc_now().date().isoformat() / run_id
        ...
        for summary in summaries:
            if summary.report_path:
                ...
                if report_path.exists():
                    archive.write(report_path, f"audit/{report_path.name}")
    copy2(
        manifest_path,
        report_root / "signal" / _utc_now().date().isoformat() / f"{run_id}-summary.json",
    )


def _closeout_text(manifest: dict[str, Any]) -> str:
    lines = [
        f"# Signal {manifest['stage']} closeout: {manifest['run_id']}",
        "",
