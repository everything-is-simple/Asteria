...
使用本技能时，Asteria 变更必须保持文档先行、模块内收敛，并与当前 live gate 顺序对齐。
...
-> full_rebuild_and_daily_incremental_release_closeout_card blocked / formal release evidence incomplete
-> formal_full_rebuild_and_daily_incremental_release_proof_card blocked / runner surface missing
```

当前 live next 只允许补 formal release evidence：formal full rebuild proof、daily incremental release proof、
resume/idempotence proof 与 final release evidence。它不授权 Pipeline semantic repair、业务模块语义重定义、
System full build 或 `v1 complete`。

正式写 `H:\Asteria-data` 只允许通过 guarded runner：`H:\Asteria-temp` staging rebuild、
formal DB backup、显式 `--allow-formal-data-write`、审计通过后 promote。任何失败都保持 blocked。
