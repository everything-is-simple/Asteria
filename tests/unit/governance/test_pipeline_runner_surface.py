...
def test_docs_sync_allows_pipeline_disposition_decision_runner(tmp_path: Path) -> None:
    repo_root = build_governance_repo(tmp_path)
    source_runner = (
        Path(__file__).resolve().parents[3]
        / "scripts"
    ...
    target_runner = (
        repo_root / "scripts" / "pipeline" / "run_pipeline_year_replay_disposition_decision.py"
    )
    target_runner.parent.mkdir(parents=True, exist_ok=True)
    target_runner.write_text(source_runner.read_text(encoding="utf-8"), encoding="utf-8")
