...
def test_docs_sync_rejects_pre_gate_trade_formal_runner(tmp_path: Path) -> None:
    repo_root = _copy_docs_sync_repo(tmp_path)
    trade_runner = repo_root / "scripts" / "trade" / "run_trade_build.py"
    trade_runner.parent.mkdir(parents=True)
    trade_runner.write_text("raise SystemExit(0)\n", encoding="utf-8")
