...
proof surface 已放行。本卡执行 review-only Trade freeze review，不执行 Trade build。
...
- review-only 审阅 Trade 六件套。
- 冻结 Trade design / schema / runner / audit 合同表面。
- 生成 Trade freeze review 的 record、evidence-index、conclusion。
...
Trade freeze review 通过后，只允许准备 Trade bounded proof build card。
