...
| Pipeline | `downstream-daily-impact-ledger-schema-card` | `passed / downstream daily impact schema frozen` | [conclusion](records/pipeline/downstream-daily-impact-ledger-schema-card.conclusion.md) | [evidence-index](records/pipeline/downstream-daily-impact-ledger-schema-card.evidence-index.md) |
...
| Signal | `signal-production-builder-hardening-20260506-01` | `passed` | [conclusion](records/signal/signal-production-builder-hardening-20260506-01.conclusion.md) | [evidence-index](records/signal/signal-production-builder-hardening-20260506-01.evidence-index.md) |
...
| Trade | `trade-2024-coverage-repair-card-20260509-01` | `completed / downstream breakpoint moved to system_readout` | [conclusion](records/trade/trade-2024-coverage-repair-card-20260509-01.conclusion.md) | [evidence-index](records/trade/trade-2024-coverage-repair-card-20260509-01.evidence-index.md) |
...
| System Readout | `system-readout-2024-coverage-repair-card-20260509-01` | `completed / calendar_semantic_gap_only` | [conclusion](records/system_readout/system-readout-2024-coverage-repair-card-20260509-01.conclusion.md) | [evidence-index](records/system_readout/system-readout-2024-coverage-repair-card-20260509-01.evidence-index.md) |
...
| Pipeline | `pipeline-single-module-orchestration-build-card-20260508-01` | `passed` | [conclusion](records/pipeline/pipeline-single-module-orchestration-build-card-20260508-01.conclusion.md) | [evidence-index](records/pipeline/pipeline-single-module-orchestration-build-card-20260508-01.evidence-index.md) |
...
- Trade bounded proof build、System Readout freeze review、System Readout bounded proof build、Pipeline freeze review、
  ...
  `trade-2024-coverage-repair-card-20260509-01`。随后
  `trade-2024-coverage-repair-card-20260509-01` 也已于 `2026-05-10` 真实执行并完成：released Trade
  rejection surface 已前移到 `2024-01-02`，released `order_intent` / `execution_plan` 已在真实 admitted day
  `2024-01-05` materialize，因此新的 live next card 已继续下移到
  `system-readout-2024-coverage-repair-card-20260509-01`。随后
  `pipeline-trade-2024-coverage-repair-handoff-20260510-01` 也已通过，把 Pipeline live handoff truth
  同步到 `system_readout_2024_coverage_repair_card`。`system-readout-2024-coverage-repair-card-20260509-01`
  ...
  但因当时 formal full rebuild proof、daily incremental release proof 与 final release evidence
  尚未形成而 blocked。随后 `formal-full-rebuild-and-daily-incremental-release-proof-card`
  已补齐 formal full rebuild proof、daily incremental release proof、resume/idempotence proof 与 final release evidence，
  并通过 guarded release proof promote 路径；随后 `final-release-closeout-card` 已核对 formal release evidence
  与当前 `H:\Asteria-data` 25 个正式 DB manifest 一致，结论为 `passed / v1 complete`。
...
当前 Phase 2 的第二张路线卡也已形成：

```text
v1-signal-export-contract-card-20260513-01 = passed / signal export contract frozen
```

它冻结 Asteria Core 输出给外部回测框架的最小信号合同：`symbol`、`timeframe`、
`signal_date`、`signal_type`、`signal_strength`、`signal_family`、`source_run_id`、
`schema_version`、`signal_rule_version`、`source_alpha_release_version`、`lineage`、
`execution_hint = T_PLUS_1_OPEN`、`execution_trade_date_policy = next_trading_day_after_signal_date`
与 `execution_price_field = open`。该卡只冻结 contract，不运行收益回测、不证明成交、
...
当前 Phase 2 的第四张路线卡也已形成：

```text
v1-vectorbt-portfolio-analytics-proof-card-20260514-01 = passed / vectorbt portfolio analytics proof completed
```

它只读消费正式 `signal.duckdb` 与 `market_base_day.duckdb`，用 `vectorbt`
执行第一版多资产矩阵组合分析 proof。结果为：`31` 只代表股、`1` 只有 active
Signal、组合矩阵完成，组合收益 `-13.2839%`，最大回撤 `-24.6748%`，完成交易
...
当前 Signal/PAS contract alignment 路线卡也已形成：

```text
v1-signal-contract-alignment-card-20260514-01 = passed / signal contract aligned
```

它新增 Signal/PAS alignment-only runtime、artifact writer 与 CLI；只读消费第 6 卡 PAS proof DB，
在 `H:\Asteria-temp` 与 `H:\Asteria-report` 生成 Signal alignment evidence。
结果为 `input_candidate_count = 4395`、`active_candidate_count = 1262`、
`formal_signal_count = 1262`、`hard_fail_count = 0`，required Signal aligned fields 无缺失，
forbidden broker / position / portfolio / fill / account / profit fields 未出现。该卡不写正式 DB、
不修改正式 `signal.duckdb`、不运行收益 proof、不接 broker；当前 live 下一卡仍保持
`none / terminal`，下一张路线卡为 `v1-alpha-pas-t-plus-one-return-proof-card`。
...
| Pipeline | `v1-signal-contract-alignment-card-20260514-01` | `passed / signal contract aligned` | [conclusion](records/pipeline/v1-signal-contract-alignment-card-20260514-01.conclusion.md) |
