...
| 当前已打开执行卡 | `Position freeze review card` |
| 当前只允许施工 | `Position freeze review` |
| 当前仍禁止 | Alpha full build、Signal full build、Position/Portfolio Plan/Trade/System/Pipeline 施工 |

`Signal bounded proof` 已基于已放行的 Alpha candidate 完成最小证明。后续只允许
Position freeze review，但仍不授权 Alpha full build、Signal full build、Position
...
```text
MALF day passed -> Alpha freeze review passed -> Alpha bounded proof passed -> Signal freeze review passed -> Signal bounded proof passed -> Position freeze review card
```
