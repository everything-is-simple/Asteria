...
截至 `trade-2024-coverage-repair-card-20260509-01` completed：
...
| 当前已准备执行卡 | `system_readout_2024_coverage_repair_card` |
| 当前只允许施工 | System Readout 2024 released day surface repair |
| 当前仍禁止 | Trade full build、System full build、Pipeline semantic repair、full rebuild、daily incremental、resume/idempotence、v1 complete |
...
随后 `portfolio-plan-2024-coverage-repair-card-20260509-01` 也已真实执行并完成：released Portfolio Plan
admission surface 已前移覆盖 `2024-01-02..2024-01-05`，而 target exposure 只在真实 admitted day
`2024-01-05` 存在，语义断点已继续下移到 Trade。随后
`trade-2024-coverage-repair-card-20260509-01` 也已真实执行并完成：released Trade rejection surface
已前移到 `2024-01-02`，而 released `order_intent` / `execution_plan` 已在真实 admitted day
`2024-01-05` materialize，因此语义断点已继续下移到 System Readout。当前 live authority 已切到
`system_readout_2024_coverage_repair_card`。当前只允许 System Readout 2024 released day surface repair，
不允许扩成 Pipeline semantic repair，或打开 full rebuild / v1 complete。
...
任何下游实现都必须等前置模块完成 freeze / proof / release evidence。当前 repair handoff 已继续下移到
Trade，但这仍然只是 released day surface 的最小修补链，不等于 Trade full build、System full build、
full rebuild 或 `v1 complete` 已通过。
