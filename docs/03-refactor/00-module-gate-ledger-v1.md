...
```text
Pipeline one-year strategy behavior replay build blocked
```
...
`2024-01-01..2024-12-31` 完整自然年覆盖不足。当前 live prepared next card 已切换为
`pipeline_year_replay_coverage_gap_diagnosis_and_repair_scope_freeze`，且仍不得扩展为 Position full build、
Portfolio Plan full build、Trade full build、System full build、full rebuild、daily incremental 或 `v1 complete`。

2026-05-09：`docs/03-refactor/05-system-validation-repair-roadmap-v1.md` 已新增为后半场系统验证与修补
roadmap。当前唯一 prepared next card 对应
`pipeline-year-replay-coverage-gap-diagnosis-and-repair-scope-freeze-20260509-01`。该卡只允许 read-only
coverage gap diagnosis，裁定 `2024-01-01..2024-01-07` 缺口归属；不允许修库、重跑 year replay、开启
...
| 8 | Pipeline | frozen six-doc set / freeze review passed / single-module orchestration build passed / full-chain dry-run passed / full-chain day bounded proof passed / one-year strategy behavior replay blocked / coverage gap diagnosis prepared | released / full-chain day bounded proof passed / year replay blocked / diagnosis prepared | 否 | `docs/02-modules/pipeline/` | 只编排和记录，不抢业务施工位；`pipeline.duckdb` 已创建，并已落下 full-chain day bounded proof ledger；year replay 已执行并留下行为摘要，但因自然年覆盖不足被 truthful blocked；当前只允许 coverage gap diagnosis |
