...
Alpha 已通过 freeze review 并冻结六件套；Alpha bounded proof build card 已打开。
当前只允许 Alpha bounded proof 施工。Signal、Position、Portfolio Plan、Trade、
System Readout 和 Pipeline 为 pre-gate six-doc draft，不允许施工。
...
| 2 | Alpha | frozen six-doc set / freeze review passed | frozen | 是 | `docs/02-modules/alpha/` | Alpha bounded proof build card 已打开；只允许 bounded proof |
...
后续 Alpha bounded proof build card 已打开，授权范围仍只限 Alpha bounded proof。

## 7. 当前施工卡

当前施工卡为 Alpha bounded proof build card，不得扩展为 full build 或下游施工：

```text
docs/04-execution/records/alpha/alpha-bounded-proof-build-card-20260429-01.card.md
```
...
| Formal DB Permission | 只允许在本卡 bounded proof/audit 路径下创建 working / formal Alpha DB |
| Downstream Lock | 不打开 Signal / Position / Portfolio / Trade / System 施工 |

## 8. 施工锁

在 Alpha bounded proof 未通过 release gate 前，除当前卡明确授权范围外，不允许：

| 禁止项 |
|---|
| 迁移旧 Alpha / Signal / Position / Portfolio / Trade / System 代码 |
| 创建 Signal / Position / Portfolio / Trade / System 正式 DB |
| 创建超出 Alpha bounded proof/audit 范围的 Alpha 正式 DB 或运行 full build |
| 让下游模块补充自有语义 |
| 建立 pipeline 全链路 |
