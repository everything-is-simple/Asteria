...
`pipeline-full-chain-bounded-proof-build-card-20260508-01` 与
`pipeline-full-chain-bounded-proof-closeout-20260508-01` 已通过；
其中 `pipeline-full-chain-bounded-proof-build-card-20260508-01` 的 allowed next action 是
`pipeline_full_chain_bounded_proof_closeout`，而
`pipeline-full-chain-bounded-proof-closeout-20260508-01` 的 allowed next action 是
`pipeline_one_year_strategy_behavior_replay_authorization_scope_freeze`；
`pipeline-one-year-strategy-behavior-replay-authorization-scope-freeze-20260508-01` 已通过，其
allowed next action 是 `pipeline_one_year_strategy_behavior_replay_build_card`；
`pipeline-one-year-strategy-behavior-replay-build-card-20260508-01` 已执行但 `blocked`，原因是
`2024-01-01..2024-12-31` 完整自然年覆盖不足。随后
`pipeline-year-replay-coverage-gap-diagnosis-and-repair-scope-freeze-20260509-01` 已执行并形成正式诊断：
Data `2024-01-02..2024-01-05` 已覆盖，但 released MALF day surface 从 `2024-01-08` 才开始，最早 released
surface break 锁定在 MALF。当前 live prepared next card 已切换为
...
`malf-2024-natural-year-coverage-repair-card-20260509-01`。随后
`malf-2024-natural-year-coverage-repair-card-20260509-01` 已通过最小 MALF released day surface repair：
对 `000020.SZ` 生成新的 released service run
...
`pipeline-one-year-strategy-behavior-replay-rerun-handoff-20260509-01` 已通过，其 allowed next action 是
`pipeline_one_year_strategy_behavior_replay_rerun_build_card`。随后
`pipeline-one-year-strategy-behavior-replay-rerun-build-card-20260509-01` 已真实执行，但结果继续
blocked：`observed_start` 仍是 `2024-01-08`，且 rerun source lock 未指向 repaired MALF run。随后
`alpha-signal-2024-coverage-repair-card-20260509-01` 已通过，released Alpha / Signal day surface 已前移到
`2024-01-02`；该结论当时的 allowed next action 是
`coverage_gap_evidence_incomplete_closeout_card`。随后
`coverage-gap-evidence-incomplete-closeout-card-20260509-01` 也已真实执行并通过，确认 released
Position / Portfolio Plan / Trade day surface 的最早日期都落在 `2024-01-09`，因此当前唯一 prepared
next card 已切到 `position-2024-coverage-repair-card-20260509-01`；该 Pipeline 结论的历史 allowed next action
为 `portfolio_plan_2024_coverage_repair_card`，随后已被 Position repair 和 Portfolio Plan repair 继续消费。
后半场 system validation / repair
...
Portfolio Plan。随后 `portfolio-plan-2024-coverage-repair-card-20260509-01` 也已真实执行并完成：released
Portfolio Plan admission surface 已前移覆盖 `2024-01-02..2024-01-05`，而 target exposure 只在真实 admitted day
`2024-01-05` 存在，因此新的 live allowed next action 已切到 `trade_2024_coverage_repair_card`；Trade
repair 之外的 stage 11 队列仍不得抢跑。

2026-05-10：`pipeline-portfolio-plan-2024-coverage-repair-handoff-20260510-01` 已通过，正式接住
Portfolio Plan repair follow-up 的 `downstream_surface_gap:trade` 归因；其 allowed next action 是
`trade_2024_coverage_repair_card`。该 handoff 只更新 Pipeline live next-card truth，不直接执行 Trade repair，
也不打开 System repair、Pipeline semantic repair、full rebuild、daily incremental 或 `v1 complete`。

2026-05-10：`trade-2024-coverage-repair-card-20260509-01` 已真实执行并完成。released Trade rejection
surface 已前移到 `2024-01-02`，而 released `order_intent` / `execution_plan` 已在真实 admitted day
`2024-01-05` materialize；因此 Trade 语义断点已真实下移到 System Readout。当前唯一 live
allowed next action 已切到 `system_readout_2024_coverage_repair_card`；这仍不打开 System full build、
Pipeline semantic repair、full rebuild、daily incremental、resume/idempotence 或 `v1 complete`。

2026-05-10：`pipeline-trade-2024-coverage-repair-handoff-20260510-01` 已通过，正式接住
Trade repair follow-up 的 `released_surface_gap:system_readout` 归因；其 allowed next action 是
`system_readout_2024_coverage_repair_card`。该 handoff 只更新 Pipeline live next-card truth，不直接执行
System Readout repair，也不打开 Pipeline semantic repair、full rebuild、daily incremental 或 `v1 complete`。

2026-05-10：`system-readout-2024-coverage-repair-card-20260509-01` 已真实执行并完成。released System Readout
day surface 已覆盖 `2024-01-02..2024-01-05`；follow-up attribution 只剩 `calendar_semantic_gap_only`。
随后 `pipeline-system-readout-2024-coverage-repair-handoff-20260510-01` 已通过，正式接住该 follow-up 归因；
...
scope freeze、Pipeline single-module orchestration build、Pipeline full-chain dry-run authorization
scope freeze 与 Pipeline full-chain dry-run 也已通过。`pipeline.duckdb` 已创建，并已落下
single-module orchestration 与 full-chain day dry-run 两条正式 run ledger。
...
| 1 | MALF | 六件套已交付 / v1.4 day runtime sync 已通过 / week/month bounded proof 已通过 / v1.4 authority sync 已通过 / 2024 natural-year coverage repair passed | released / minimal repair passed | 否 | `docs/02-modules/malf/` | `000020.SZ` 的 repaired released run 已补到 `2024-01-02..2024-01-05`；MALF full build 与 20-symbol 全量重修仍需另开卡 |
...
| 4 | Position | 六件套 freeze review passed / release decision passed / bounded proof passed / 2024 coverage repair passed | released / full build not executed | 否 | `docs/02-modules/position/` | day bounded proof 已通过；2024 repair 已真实执行并把 candidate day surface 前移到 `2024-01-02`；`2024-01-02..2024-01-03` 的 released Signal 仍是 rejected，因此 Position 语义断点已真实下移到 Portfolio Plan |
| 5 | Portfolio Plan | 六件套 freeze review passed / bounded proof passed / 2024 coverage repair passed / full build not executed | released / full build not executed | 否 | `docs/02-modules/portfolio_plan/` | day bounded proof 已通过；2024 repair 已真实执行并把 admission surface 前移覆盖 `2024-01-02..2024-01-05`，当前 live 下一断点已下移到 Trade；Portfolio Plan full build 仍需另开卡 |
| 6 | Trade | 六件套 freeze review passed / bounded proof passed / 2024 coverage repair passed / full build not executed | released / 2024 coverage repair passed / full build not executed | 否 | `docs/02-modules/trade/` | Trade bounded proof 已通过；2024 repair 已真实执行并把 rejection surface 前移到 `2024-01-02`，并在真实 admitted day `2024-01-05` materialize `order_intent` / `execution_plan`；当前 live 下一断点已下移到 System Readout |
| 7 | System Readout | frozen six-doc set / freeze review passed / bounded proof passed / 2024 coverage repair passed / full build not executed | released / bounded proof passed / 2024 coverage repair passed | 否 | `docs/02-modules/system_readout/` | System Readout 2024 released day surface repair 已真实执行并完成；follow-up attribution 只剩 `calendar_semantic_gap_only`，当前 live 下一卡已切到 Pipeline source selection repair |
| 8 | Pipeline | frozen six-doc set / freeze review passed / single-module orchestration build passed / full-chain dry-run passed / full-chain day bounded proof passed / one-year strategy behavior replay blocked / coverage gap diagnosis executed / MALF repair passed / year replay rerun blocked / alpha-signal coverage repair passed / downstream coverage gap evidence closeout passed / position 2024 coverage repair passed / portfolio plan 2024 coverage repair passed / trade 2024 coverage repair passed / live next card moved to system_readout repair / system_readout handoff passed | released / full-chain day bounded proof passed / year replay blocked / rerun blocked | 否 | `docs/02-modules/pipeline/` | 只编排和记录，不抢业务施工位；`pipeline.duckdb` 已创建，并已落下 full-chain day bounded proof ledger；当前 live 下一步已收口到 Pipeline source selection repair |
...
| `system-readout-freeze-review-20260507-01` | `passed` | `system_readout_bounded_proof_build_card` |
| `system-readout-bounded-proof-build-card-20260508-01` | `passed` | `pipeline_freeze_review` |
| `pipeline-freeze-review-20260508-01` | `passed` | `none` |
| `pipeline-build-runtime-authorization-scope-freeze-20260508-01` | `passed / scope frozen` | `pipeline_single_module_orchestration_build_card` |
| `pipeline-single-module-orchestration-build-card-20260508-01` | `passed` | `none` |
| `pipeline-full-chain-dry-run-authorization-scope-freeze-20260508-01` | `passed / scope frozen` | `pipeline_full_chain_dry_run_card` |
| `pipeline-full-chain-dry-run-card-20260508-01` | `passed` | `none` |
...
| System Readout | `docs/02-modules/system_readout/00-authority-design-v1.md` | frozen / freeze review passed / bounded proof passed / full build not executed |
| System Readout | `docs/02-modules/system_readout/01-semantic-contract-v1.md` | frozen / freeze review passed / bounded proof passed / full build not executed |
| System Readout | `docs/02-modules/system_readout/02-database-schema-spec-v1.md` | frozen / freeze review passed / bounded proof passed / full build not executed |
...
| Pipeline | `docs/02-modules/pipeline/00-authority-design-v1.md` | frozen / freeze review passed / single-module orchestration build passed / full-chain dry-run passed |
| Pipeline | `docs/02-modules/pipeline/01-semantic-contract-v1.md` | frozen / freeze review passed / single-module orchestration build passed / full-chain dry-run passed |
| Pipeline | `docs/02-modules/pipeline/02-database-schema-spec-v1.md` | frozen / freeze review passed / single-module orchestration build passed / full-chain dry-run passed |
| Pipeline | `docs/02-modules/pipeline/03-runner-contract-v1.md` | frozen / freeze review passed / single-module orchestration build passed / full-chain dry-run passed |
| Pipeline | `docs/02-modules/pipeline/04-audit-spec-v1.md` | frozen / freeze review passed / single-module orchestration build passed / full-chain dry-run passed |
| Pipeline | `docs/02-modules/pipeline/05-build-card-v1.md` | frozen / freeze review passed / single-module orchestration build passed / full-chain dry-run passed |
...
已通过。Trade freeze review、Trade bounded proof build、System Readout freeze review 与 System Readout bounded proof build 已通过。当前下一步唯一允许动作已改为 Pipeline freeze review；Trade full build、
Portfolio Plan full build、Position full build、System full build、Pipeline runtime 仍不允许直接施工。
