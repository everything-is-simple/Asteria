...
```text
Pipeline year replay coverage gap diagnosis passed
```
...
```text
MALF 2024 natural-year coverage repair
```
...
阶段已收敛回 `docs/03-refactor/04-asteria-full-system-roadmap-v1.md` 单点维护；旧第二张 repair roadmap
已删除，不再作为 roadmap 权威入口。该卡只允许最小 MALF released day surface repair；不允许扩成
...
| 1 | MALF | 六件套已交付 / v1.4 day runtime sync 已通过 / week/month bounded proof 已通过 / v1.4 authority sync 已通过 / 2024 natural-year coverage repair prepared | frozen / repair prepared | 是，仅限当前 prepared repair card | `docs/02-modules/malf/` | 当前 live repair 只允许补 `2024-01-02..2024-01-05` released day surface 覆盖；full build 仍需另开卡 |
...
| 8 | Pipeline | frozen six-doc set / freeze review passed / single-module orchestration build passed / full-chain dry-run passed / full-chain day bounded proof passed / one-year strategy behavior replay blocked / coverage gap diagnosis executed / MALF repair prepared | released / full-chain day bounded proof passed / year replay blocked / diagnosis executed / MALF repair prepared | 否 | `docs/02-modules/pipeline/` | 只编排和记录，不抢业务施工位；`pipeline.duckdb` 已创建，并已落下 full-chain day bounded proof ledger；year replay 已执行并留下行为摘要；coverage gap diagnosis 已证明最早 released surface break 在 MALF，当前只允许最小 MALF repair |
