...
next route card = v1-t-plus-one-open-backtesting-py-proof-card
```
...
进入日更生产化范围冻结前，必须先消费 `v1-t-plus-one-open-backtesting-py-proof-card`
对 `T+0 signal -> T+1 open execution` 的外部回测 proof。
...
```text
final release closeout 已通过；当前没有下一张 live card
```
...
| 8 | Pipeline | frozen six-doc set / freeze review passed / single-module orchestration build passed / full-chain dry-run passed / full-chain day bounded proof passed / one-year strategy behavior replay blocked / repair chain closed / stage 11 daily incremental samples passed / formal release proof passed / final release closeout passed | released / v1 complete | 否 | `docs/02-modules/pipeline/` | 只编排和记录，不抢业务施工位；`pipeline.duckdb` 已创建，并已落下 full-chain day bounded proof ledger；当前无下一张 live card |
