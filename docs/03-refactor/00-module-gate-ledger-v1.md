...
```text
Pipeline one-year strategy behavior replay rerun handoff passed
```
...
```text
pipeline-one-year-strategy-behavior-replay-rerun-handoff-20260509-01
...
```text
Alpha / Signal 2024 coverage repair
```
...
blocked：`observed_start` 仍是 `2024-01-08`，且 rerun source lock 未指向 repaired MALF run。当前唯一 prepared
next card 已切到 `alpha-signal-2024-coverage-repair-card-20260509-01`。后半场 system validation / repair
阶段继续收敛回 `docs/03-refactor/04-asteria-full-system-roadmap-v1.md` 单点维护；旧第二张 repair roadmap
已删除，不再作为 roadmap 权威入口。当前只允许 Alpha / Signal 2024 coverage repair；不允许扩成 System / Pipeline repair，
...
| 8 | Pipeline | frozen six-doc set / freeze review passed / single-module orchestration build passed / full-chain dry-run passed / full-chain day bounded proof passed / one-year strategy behavior replay blocked / coverage gap diagnosis executed / MALF repair passed / year replay rerun blocked / alpha-signal coverage repair prepared | released / full-chain day bounded proof passed / year replay blocked / rerun blocked | 否 | `docs/02-modules/pipeline/` | 只编排和记录，不抢业务施工位；`pipeline.duckdb` 已创建，并已落下 full-chain day bounded proof ledger；rerun 已证明 released Alpha/Signal 观察链仍未跟上 repaired MALF source，因此当前下一步转为 Alpha/Signal repair |
