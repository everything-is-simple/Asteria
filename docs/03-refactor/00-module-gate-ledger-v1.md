...
`2024-01-01..2024-12-31` 完整自然年覆盖不足。当前没有 live prepared next card，且仍不得扩展为
Position full build、Portfolio Plan full build、Trade full build、System full build、full rebuild、
...
| 8 | Pipeline | frozen six-doc set / freeze review passed / single-module orchestration build passed / full-chain dry-run passed / full-chain day bounded proof passed / one-year strategy behavior replay blocked | released / full-chain day bounded proof passed / year replay blocked | 否 | `docs/02-modules/pipeline/` | 只编排和记录，不抢业务施工位；`pipeline.duckdb` 已创建，并已落下 full-chain day bounded proof ledger；year replay 已执行并留下行为摘要，但因自然年覆盖不足被 truthful blocked |
