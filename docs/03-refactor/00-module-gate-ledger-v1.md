...
```text
Pipeline one-year strategy behavior replay rerun
```
...
四个 focus trading dates 在 repaired MALF released run 上可见，且 `hard_fail_count = 0`。当前唯一 prepared
next card 已切到 `pipeline-one-year-strategy-behavior-replay-rerun-build-card-20260509-01`。后半场 system validation / repair
阶段继续收敛回 `docs/03-refactor/04-asteria-full-system-roadmap-v1.md` 单点维护；旧第二张 repair roadmap
已删除，不再作为 roadmap 权威入口。当前只允许 year replay rerun；不允许扩成 Alpha / Signal / System / Pipeline repair，
...
| 8 | Pipeline | frozen six-doc set / freeze review passed / single-module orchestration build passed / full-chain dry-run passed / full-chain day bounded proof passed / one-year strategy behavior replay blocked / coverage gap diagnosis executed / MALF repair passed / year replay rerun prepared | released / full-chain day bounded proof passed / year replay blocked / rerun prepared | 否 | `docs/02-modules/pipeline/` | 只编排和记录，不抢业务施工位；`pipeline.duckdb` 已创建，并已落下 full-chain day bounded proof ledger；year replay 已执行并留下行为摘要；coverage gap diagnosis 已证明最早 released surface break 在 MALF，当前只允许 rerun 锁到新的 MALF repaired run |
