...
也已完成 `day`-only 协议冻结，当前 live `current_allowed_next_card`
已切到 `malf_daily_incremental_ledger_build_card`，中间的
`data-ledger-daily-incremental-hardening-card` 已通过 Data-only 样板 proof。
...
- 当前唯一 live next card 只允许 `malf-daily-incremental-ledger-build-card`，不得扩成 Alpha/Signal、下游 daily runtime、full rebuild、Pipeline semantic repair 或 `v1 complete`
...
| 优先级 | 候选卡 | 说明 |
|---|---|---|
| P0 | `malf-daily-incremental-ledger-build-card` | Data daily incremental 样板已通过后，当前唯一 live next card |
...
| 层级 | 作用 | 当前内容 |
|---|---|---|
| 层 1 | 当前 live next card | `malf-daily-incremental-ledger-build-card` |
...
- 当前 live `current_allowed_next_card` 是 `system_wide_daily_dirty_scope_protocol_card`；Pipeline 已通过
  ...
  当前 live `current_allowed_next_card` 已继续前进到 `malf_daily_incremental_ledger_build_card`；
  这仍不是 Alpha/Signal、下游 daily runtime、System full build、Pipeline semantic repair、full rebuild 或 `v1 complete` 授权。
