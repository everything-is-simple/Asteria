...
Portfolio Plan freeze review 已通过。当前下一步只允许
`portfolio_plan_bounded_proof_build_card`，Portfolio Plan build 尚未执行。
...
hardening、Signal production hardening、upstream release decision、Position bounded proof 与
Portfolio Plan freeze review 已闭环；当前 allowed next action 已切到
`portfolio_plan_bounded_proof_build_card`。
...
- [x] 冻结 `portfolio_plan.duckdb` schema 合同；正式 DB 仍等待 bounded proof build card。
...
- 当前下一卡固定为 `portfolio_plan_bounded_proof_build_card`；这只准备 Portfolio Plan bounded proof，不是 full build 或下游施工。
