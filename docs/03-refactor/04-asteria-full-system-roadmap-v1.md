...
evidence 尚未形成，因此当前 release closeout 结论为 blocked。
...
| 优先级 | 候选卡 | 说明 |
|---|---|---|
| P0 | `full-rebuild-and-daily-incremental-release-closeout-card` | 已 executed / blocked；需要真实 full rebuild 与 release evidence 才能解除 |
...
| 层级 | 作用 | 当前内容 |
|---|---|---|
| 层 1 | 当前 release closeout 状态 | `full-rebuild-and-daily-incremental-release-closeout-card` = blocked / formal release evidence incomplete |
...
- 当前 live `current_allowed_next_card` 是 `system_wide_daily_dirty_scope_protocol_card`；Pipeline 已通过
  ...
  `pipeline-full-daily-incremental-chain-build-card` 也已真实执行并完成，当前唯一 live next card
  已继续前进到 `full_rebuild_and_daily_incremental_release_closeout_card`；这仍不是 formal full rebuild
  passed、daily incremental release closeout passed、System full build 或 `v1 complete` 授权。
