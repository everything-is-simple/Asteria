...
与 `pipeline-full-chain-dry-run-authorization-scope-freeze-20260508-01` 已通过。当前 allowed next action 为
`pipeline_full_chain_dry_run_card`；full-chain dry-run 已准备但未执行，full-chain bounded proof 仍必须另开新卡。
...
- [ ] 在 `pipeline-full-chain-dry-run-card-20260508-01` 中实现全链路 dry-run。
...
- 当前 allowed next action 为 `pipeline_full_chain_dry_run_card`；Pipeline 已通过 `system_readout` 单模块 orchestration 证据，并已准备 full-chain dry-run 下一张卡，但这仍不是 System full build、full-chain dry-run passed、full-chain bounded proof 或下游扩展施工授权。
