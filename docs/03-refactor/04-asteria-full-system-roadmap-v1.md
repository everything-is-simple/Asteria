...
也已通过 MALF day-only 样板 proof。当前 live `current_allowed_next_card`
已切到 `downstream_daily_incremental_runner_build_card`。
...
- 当前唯一 live next card 只允许 `downstream-daily-incremental-runner-build-card`，不得扩成 full daily chain、full rebuild、Pipeline semantic repair 或 `v1 complete`
...
| 优先级 | 候选卡 | 说明 |
|---|---|---|
| P0 | `downstream-daily-incremental-runner-build-card` | downstream impact schema 已冻结后，当前唯一 live next card |
...
| 层级 | 作用 | 当前内容 |
|---|---|---|
| 层 1 | 当前 live next card | `downstream-daily-incremental-runner-build-card` |
...
| 6 | `downstream-daily-incremental-runner-build-card` | 当前 live next：实现下游按 daily impact scope 重算、断点续传、幂等 promote |
| 7 | `pipeline-full-daily-incremental-chain-build-card` | Pipeline 编排 Data -> MALF -> Alpha -> Signal -> Position -> Portfolio Plan -> Trade -> System |
...
- 当前 live `current_allowed_next_card` 是 `system_wide_daily_dirty_scope_protocol_card`；Pipeline 已通过
  ...
  `downstream-daily-impact-ledger-schema-card` 也已真实执行并完成，当前唯一 live next card 已继续前进到
  `downstream_daily_incremental_runner_build_card`；这仍不是 Pipeline full daily chain、System full build、
  full rebuild 或 `v1 complete` 授权。
