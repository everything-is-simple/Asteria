...
也已通过 MALF day-only 样板 proof。随后 downstream day-only runner 样板也已闭环，
当前 live `current_allowed_next_card` 已切到 `pipeline_full_daily_incremental_chain_build_card`。
...
- 当前唯一 live next card 只允许 `pipeline-full-daily-incremental-chain-build-card`，不得跳成 formal full rebuild、release closeout、Pipeline semantic repair 或 `v1 complete`
...
| 优先级 | 候选卡 | 说明 |
|---|---|---|
| P0 | `pipeline-full-daily-incremental-chain-build-card` | downstream day-only runner 样板已通过后，当前唯一 live next card |
...
| 层级 | 作用 | 当前内容 |
|---|---|---|
| 层 1 | 当前 live next card | `pipeline-full-daily-incremental-chain-build-card` |
...
| 7 | `pipeline-full-daily-incremental-chain-build-card` | 当前 live next：Pipeline 编排 Data -> MALF -> Alpha -> Signal -> Position -> Portfolio Plan -> Trade -> System |
| 8 | `full-rebuild-and-daily-incremental-release-closeout-card` | 运行 full rebuild proof、daily incremental proof、resume/idempotence proof，并生成 final release evidence |
...
- 当前 live `current_allowed_next_card` 是 `system_wide_daily_dirty_scope_protocol_card`；Pipeline 已通过
  ...
  `downstream-daily-incremental-runner-build-card` 也已真实执行并完成，当前唯一 live next card 已继续前进到
  `pipeline_full_daily_incremental_chain_build_card`；这仍不是 formal full rebuild、
  daily incremental release closeout、System full build 或 `v1 complete` 授权。
