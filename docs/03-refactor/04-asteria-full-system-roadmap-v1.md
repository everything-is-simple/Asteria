...
`alpha-signal-2024-coverage-repair-card-20260509-01` 现已通过，当前 live `current_allowed_next_card`
已切到 `position_2024_coverage_repair_card`。
...
- `position-2024-coverage-repair-card-20260509-01` 已于 `2026-05-10` 真实执行；released Position candidate day surface 已前移到 `2024-01-02`，但 entry / exit 仍从 `2024-01-04` 起步，follow-up attribution 仍是 `downstream_surface_gap:position`
- 当前唯一 live next card 仍只允许 Position 2024 released day surface repair，不得扩成 Portfolio Plan / Trade / System / Pipeline repair
...
| 优先级 | 候选卡 | 说明 |
|---|---|---|
| P0 | `position-2024-coverage-repair-card-20260509-01` | 已执行一次但未完成下移；当前仍是唯一 live next card |
| P1 | `portfolio-plan-2024-coverage-repair-card-20260509-01` | 仅在 Position 2024 repair 后仍证明下一断点已下移到 Portfolio Plan 时才可进入 live authority |
| P1 | `trade-2024-coverage-repair-card-20260509-01` | 仅在 Position / Portfolio Plan 读面齐后仍证明首断点位于 Trade 时才可进入 live authority |
| P2 | `pipeline-one-year-strategy-behavior-replay-rerun-build-card-20260509-01` | 已 blocked，不再是 prepared next card |
...
| 顺序 | 候选卡 | 目标 |
|---:|---|---|
| 1 | `position-2024-coverage-repair-card-20260509-01` | 已执行一次；当前仍需在 Position 边界内继续收口 released day surface，再决定是否继续下移到 Portfolio Plan / Trade repair |
...
- 当前 live `current_allowed_next_card` 是 `position_2024_coverage_repair_card`；Pipeline 已通过
  ...
  `position-2024-coverage-repair-card-20260509-01` 已执行一次但 follow-up 仍锁定 `downstream_surface_gap:position`。
  下一步只允许继续 Position 2024 released day surface repair；这仍不是 System full build、full rebuild、daily incremental 或 `v1 complete` 授权。
