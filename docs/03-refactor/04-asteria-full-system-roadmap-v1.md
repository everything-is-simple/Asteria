...
已切到 `downstream_daily_impact_ledger_schema_card`。
...
- 当前唯一 live next card 只允许 `downstream-daily-impact-ledger-schema-card`，不得扩成下游 daily runtime、full rebuild、Pipeline semantic repair 或 `v1 complete`
...
| 优先级 | 候选卡 | 说明 |
|---|---|---|
| P0 | `downstream-daily-impact-ledger-schema-card` | Alpha/Signal daily incremental 样板已通过后，当前唯一 live next card |
...
| 层级 | 作用 | 当前内容 |
|---|---|---|
| 层 1 | 当前 live next card | `downstream-daily-impact-ledger-schema-card` |
...
- 当前 live `current_allowed_next_card` 是 `system_wide_daily_dirty_scope_protocol_card`；Pipeline 已通过
  ...
  当前 live `current_allowed_next_card` 已继续前进到 `downstream_daily_impact_ledger_schema_card`；
