...
已切到 `coverage_gap_evidence_incomplete_closeout_card`。
...
- temp system probe diagnosis 已把新的首断点锁到 downstream `position`
- 当前唯一 prepared next card 只允许 downstream coverage gap evidence closeout，不得扩成 Data / System / Pipeline repair
...
| 优先级 | 候选卡 | 说明 |
|---|---|---|
| P0 | `coverage-gap-evidence-incomplete-closeout-card-20260509-01` | 当前唯一 prepared next card |
| P1 | `system-readout-2024-coverage-repair-card-20260509-01` | 仅在 downstream closeout 后仍证明 System 读面本身缺口独立存在时才可进入 live authority |
| P1 | `pipeline-year-replay-source-selection-repair-card-20260509-01` | 仅在 downstream closeout 后证明 released surface 已齐但 Pipeline 年门禁语义仍错时才可进入 live authority |
...
| 顺序 | 候选卡 | 目标 |
|---:|---|---|
| 1 | `coverage-gap-evidence-incomplete-closeout-card-20260509-01` | 当前唯一 prepared next card；先把 downstream released day surface 缺口收口，再决定是否拆成 Position / Portfolio / Trade repair |
...
- 当前 live `current_allowed_next_card` 是 `coverage_gap_evidence_incomplete_closeout_card`；Pipeline 已通过
  ...
  与 `alpha-signal-2024-coverage-repair-card-20260509-01` 也都已通过最小 repair；新的 downstream 首断点已落到 `position`。
  下一步只允许 downstream coverage gap evidence closeout；这仍不是 System full build、full rebuild、daily incremental 或 `v1 complete` 授权。
