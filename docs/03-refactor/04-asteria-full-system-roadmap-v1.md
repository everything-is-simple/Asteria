...
`trade-2024-coverage-repair-card-20260509-01` 现已通过；`pipeline-year-replay-disposition-decision-card-20260510-01`
也已完成 truthful closeout 裁决；随后 `system-wide-daily-dirty-scope-protocol-card`
也已完成 `day`-only 协议冻结；`data-ledger-daily-incremental-hardening-card`
已通过 Data-only 样板 proof；`malf-daily-incremental-ledger-build-card`
也已通过 MALF day-only 样板 proof。随后 downstream day-only runner 样板与 Pipeline full daily
incremental chain proof 也已闭环；随后 `full-rebuild-and-daily-incremental-release-closeout-card`
已执行 truthful closeout，但当时 formal full rebuild proof、daily incremental release proof 与 final release
evidence 尚未形成，因此该 closeout 结论为 blocked。随后
`formal-full-rebuild-and-daily-incremental-release-proof-card` 已补齐 formal release 证据并通过 guarded
promote / resume-idempotence proof。当前 live 下一卡已切到 `final-release-closeout-card`，只做最终 release
closeout / `v1 complete` 裁决。
...
- `pipeline-year-replay-source-selection-repair-card-20260509-01` 已真实执行并完成，确认 released `system_readout_run` 不变但 source-selection 已切到 repaired MALF / downstream released truth，并把 allowed next action 同步为 `pipeline_year_replay_disposition_decision_card`
- `system-wide-daily-dirty-scope-protocol-card` 已真实执行并完成：当前只冻结 `day` 主链 dirty scope / impact scope / checkpoint / lineage / writer-read-only 边界，不打开 daily runtime
- `data-ledger-daily-incremental-hardening-card` 已真实执行并完成：Data 四个行情账本的 daily incremental / resume / audit evidence 已形成样板，但未修改正式 `H:\Asteria-data`
- `malf-daily-incremental-ledger-build-card` 已真实执行并完成：MALF `day` 三库样板已正式接住 Data dirty scope，并生成 MALF 自身 `daily_impact_scope` / lineage / checkpoint / audit，但未修改正式 `H:\Asteria-data`
- `downstream-daily-impact-ledger-schema-card` 已真实执行并完成：Position / Portfolio Plan / Trade / System Readout 的 Stage 11 impact map 与 replay/checkpoint 口径已冻结，但未打开 downstream daily runtime，也未修改正式 `H:\Asteria-data`
- `downstream-daily-incremental-runner-build-card` 已真实执行并完成：下游四模块 day-only sample runner 已串通，但未修改正式 `H:\Asteria-data`
- `pipeline-full-daily-incremental-chain-build-card` 已真实执行并完成：Pipeline 已按 Data -> MALF -> Alpha -> Signal -> Position -> Portfolio Plan -> Trade -> System Readout 串联 day-only daily incremental 样板
- `full-rebuild-and-daily-incremental-release-closeout-card` 已真实执行，但结论为 blocked：formal full rebuild proof、daily incremental release proof、resume/idempotence release proof 与 final release evidence 尚未形成
- `formal-full-rebuild-and-daily-incremental-release-proof-card` 已真实执行并通过：formal full rebuild proof、daily incremental release proof、resume/idempotence proof 与 final release evidence 已形成，并通过 guarded promote 路径
- 当前仍不得跳成 Pipeline semantic repair、System full build 或 `v1 complete`；下一步只能做 `final-release-closeout-card`
...
| 优先级 | 候选卡 | 说明 |
|---|---|---|
| P0 | `final-release-closeout-card` | 当前 live next；核对 formal release evidence，做最终 release closeout / `v1 complete` 裁决 |
| P1 | `pipeline-full-daily-incremental-chain-build-card` | 已 completed；不再是 live next card |
| P1 | `pipeline-one-year-strategy-behavior-replay-rerun-build-card-20260509-01` | 仅作近程 blocked backlog 参考，不得压过当前唯一下一卡 |
| P1 | `pipeline-year-replay-source-selection-repair-card-20260509-01` | 已 completed；不再是 live next card |
...
| 层级 | 作用 | 当前内容 |
|---|---|---|
| 层 1 | 当前 release proof 状态 | `formal-full-rebuild-and-daily-incremental-release-proof-card` = passed / formal release evidence complete；live next = `final-release-closeout-card` |
| 层 2 | 近程 backlog candidates | `pipeline-one-year-strategy-behavior-replay-rerun-build-card-20260509-01`，`pipeline-year-replay-source-selection-repair-card-20260509-01` |
| 层 3 | 长期阶段 11 队列 | `system-wide-daily-dirty-scope-protocol-card`，`data-ledger-daily-incremental-hardening-card`，`malf-daily-incremental-ledger-build-card`，`alpha-signal-daily-incremental-ledger-build-card`，`downstream-daily-impact-ledger-schema-card`，`downstream-daily-incremental-runner-build-card`，`pipeline-full-daily-incremental-chain-build-card`，`full-rebuild-and-daily-incremental-release-closeout-card`，`formal-full-rebuild-and-daily-incremental-release-proof-card`，`final-release-closeout-card` |
...
阶段 11 施工队列当前已被 disposition decision 正式放行为 live authority，后续逐卡进入：

| 顺序 | 候选卡 | 目标 |
|---:|---|---|
| 1 | `system-wide-daily-dirty-scope-protocol-card` | 冻结跨模块 dirty scope、daily impact scope、checkpoint、source_run lineage、writer/read-only 规则 |
| 2 | `data-ledger-daily-incremental-hardening-card` | 已通过：将 Data 4 个行情账本的 daily incremental / resume / audit evidence 做成样板 |
| 3 | `malf-daily-incremental-ledger-build-card` | 让 MALF 正式消费 Data dirty scope，并生成 MALF 自身 daily impact scope |
| 4 | `alpha-signal-daily-incremental-ledger-build-card` | 将 Alpha/Signal 接入 `symbol + date + source_run_id` 脏域传播 |
| 5 | `downstream-daily-impact-ledger-schema-card` | 已通过：为 Position/Portfolio Plan/Trade/System 冻结 impact map 与必要日期维字段 |
| 6 | `downstream-daily-incremental-runner-build-card` | 已通过：实现下游按 daily impact scope 重算、断点续传、temp-only 审计闭环 |
| 7 | `pipeline-full-daily-incremental-chain-build-card` | 已通过：Pipeline 编排 Data -> MALF -> Alpha -> Signal -> Position -> Portfolio Plan -> Trade -> System |
| 8 | `full-rebuild-and-daily-incremental-release-closeout-card` | 已执行 truthful closeout：blocked，缺 formal full rebuild proof、daily incremental proof、resume/idempotence proof 与 final release evidence |
| 9 | `formal-full-rebuild-and-daily-incremental-release-proof-card` | 已通过：formal full rebuild proof、daily incremental release proof、resume/idempotence proof 与 final release evidence complete |
| 10 | `final-release-closeout-card` | 当前 live next：最终 release closeout / `v1 complete` 裁决；如证据不一致则 truthful blocked |
...
- [x] Data/MALF 样板证明每日 dirty scope 可重跑、可 resume、可幂等 promote。
...
- [x] Pipeline 证明 full daily chain 可以从中断点恢复，且不会重复 promote 或跨库假装原子事务。
- [x] 运行 full rebuild proof。
- [x] 运行 daily incremental proof。
- [x] 运行恢复测试：中断后 resume，重复运行幂等。
- [x] 生成 final release evidence：DB manifest、schema versions、rule versions、row counts、audit summaries、known limits。
- [ ] 在上述四类 formal release evidence 全部通过后，另开 final release closeout / `v1 complete` 裁决。
...
- 当时 live `current_allowed_next_card` 是 `system_wide_daily_dirty_scope_protocol_card`；Pipeline 已通过
  ...
  `calendar_semantic_gap_only`。随后 `pipeline-year-replay-source-selection-repair-card-20260509-01`
  与 `pipeline-year-replay-disposition-decision-card-20260510-01` 也都已真实执行并完成。当前正式裁决是：
  不再重跑 rerun，而是做 truthful closeout，并把 current live `current_allowed_next_card` 切到
  `system_wide_daily_dirty_scope_protocol_card`。随后 `system-wide-daily-dirty-scope-protocol-card`
  也已真实执行并完成，当前 live `current_allowed_next_card` 已继续前进到
  `data_ledger_daily_incremental_hardening_card`。随后 Data daily hardening 已按样板 proof 通过，
  当前 live `current_allowed_next_card` 已继续前进到 `downstream_daily_impact_ledger_schema_card`；随后
  `downstream-daily-impact-ledger-schema-card` 也已真实执行并完成；随后
  `downstream-daily-incremental-runner-build-card` 也已真实执行并完成；随后
  `pipeline-full-daily-incremental-chain-build-card` 也已真实执行并完成，随后
  `full-rebuild-and-daily-incremental-release-closeout-card` 已 truthful blocked；随后
  `formal_full_rebuild_and_daily_incremental_release_proof_card` 已通过 formal release proof。当前唯一 live next card
  已继续前进到 `final_release_closeout_card`。
  这仍不是 System full build、Pipeline semantic repair 或 `v1 complete` 授权。
