...
已切到 `alpha_signal_daily_incremental_ledger_build_card`。
...
- 当前唯一 live next card 只允许 `alpha-signal-daily-incremental-ledger-build-card`，不得扩成下游 daily runtime、full rebuild、Pipeline semantic repair 或 `v1 complete`
...
| 优先级 | 候选卡 | 说明 |
|---|---|---|
| P0 | `alpha-signal-daily-incremental-ledger-build-card` | MALF daily incremental 样板已通过后，当前唯一 live next card |
...
| 层级 | 作用 | 当前内容 |
|---|---|---|
| 层 1 | 当前 live next card | `alpha-signal-daily-incremental-ledger-build-card` |
...
| 5 | `downstream-daily-impact-ledger-schema-card` | 为 Position/Portfolio Plan/Trade/System 冻结 impact map 与必要日期维字段 |
| 6 | `downstream-daily-incremental-runner-build-card` | 实现下游按 daily impact scope 重算、断点续传、幂等 promote |
...
- 当前 live `current_allowed_next_card` 是 `system_wide_daily_dirty_scope_protocol_card`；Pipeline 已通过
  ...
  当前 live `current_allowed_next_card` 已继续前进到 `alpha_signal_daily_incremental_ledger_build_card`；
  这仍不是下游 daily runtime、System full build、Pipeline semantic repair、full rebuild 或 `v1 complete` 授权。
