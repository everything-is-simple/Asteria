...
当前 live `current_allowed_next_card` 已切到 `alpha_signal_2024_coverage_repair_card`。
...
- 当前唯一 prepared next card 只允许 Alpha / Signal 2024 coverage repair，不得扩成 Data / System / Pipeline repair

本阶段固定节奏：

```text
诊断归因 -> 最小 repair -> year replay rerun -> Alpha/Signal repair -> rerun closeout -> behavior quality review
```
...
| 优先级 | 候选卡 | 说明 |
|---|---|---|
| P0 | `alpha-signal-2024-coverage-repair-card-20260509-01` | 当前唯一 prepared next card |
...
| 顺序 | 候选卡 | 目标 |
|---:|---|---|
| 1 | `alpha-signal-2024-coverage-repair-card-20260509-01` | 当前唯一 prepared next card；先补齐 released Alpha/Signal coverage，再决定是否需要 System/Pipeline repair |
...
- 当前 live `current_allowed_next_card` 是 `alpha_signal_2024_coverage_repair_card`；Pipeline 已通过
  ...
  也已通过最小 MALF repair；但 rerun 继续证明 released Alpha/Signal coverage 仍不足。下一步只允许 Alpha / Signal 2024 coverage repair；
  这仍不是 System full build、full rebuild、daily incremental 或 `v1 complete` 授权。
