...
- 当前唯一 prepared next card 只允许 Position 2024 released day surface repair，不得扩成 Portfolio Plan / Trade / System / Pipeline repair
...
| 优先级 | 候选卡 | 说明 |
|---|---|---|
| P0 | `position-2024-coverage-repair-card-20260509-01` | 当前唯一 prepared next card |
...
| 顺序 | 候选卡 | 目标 |
|---:|---|---|
| 1 | `position-2024-coverage-repair-card-20260509-01` | 当前唯一 prepared next card；先把 released Position day surface 缺口收口，再决定是否继续下移到 Portfolio Plan / Trade repair |
...
- 当前 live `current_allowed_next_card` 是 `position_2024_coverage_repair_card`；Pipeline 已通过
  ...
  下一步只允许 Position 2024 released day surface repair；这仍不是 System full build、full rebuild、daily incremental 或 `v1 complete` 授权。
