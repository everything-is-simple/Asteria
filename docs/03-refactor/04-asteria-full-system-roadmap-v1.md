...
已切到 `system_readout_2024_coverage_repair_card`。
...
- 当前唯一 live next card 只允许 System Readout 2024 released day surface repair，不得扩成 Pipeline repair
...
| 顺序 | 候选卡 | 目标 |
|---:|---|---|
| 1 | `system-readout-2024-coverage-repair-card-20260509-01` | Trade repair 已通过；当前需在 System Readout 边界内继续收口 released day surface |
...
- 当前 live `current_allowed_next_card` 是 `system_readout_2024_coverage_repair_card`；Pipeline 已通过
  ...
  `system-readout-2024-coverage-repair-card-20260509-01`。下一步只允许继续 System Readout 2024 released day
  surface repair；这仍不是 System full build、Pipeline semantic repair、full rebuild、daily incremental 或 `v1 complete` 授权。
