...
已切到 `pipeline_year_replay_disposition_decision_card`。
...
| 优先级 | 候选卡 | 说明 |
|---|---|---|
| P0 | `pipeline-year-replay-disposition-decision-card-20260510-01` | source-selection repair 已真实收口后，当前唯一 live next card |
...
| 层级 | 作用 | 当前内容 |
|---|---|---|
| 层 1 | 当前 live next card | `pipeline-year-replay-disposition-decision-card-20260510-01` |
...
阶段 11 施工队列初稿如下，必须在当前 disposition decision、year replay rerun/closeout 裁决完成之后，逐卡进入 live authority：

| 顺序 | 候选卡 | 目标 |
|---:|---|---|
| 1 | `pipeline-year-replay-disposition-decision-card-20260510-01` | 基于修正后的 released source-selection truth，裁定 rerun、closeout 或 Stage 11 路径 |
...
- 当前 live `current_allowed_next_card` 是 `pipeline_year_replay_disposition_decision_card`；Pipeline 已通过
  ...
  也已真实执行并完成，并已把 current live `current_allowed_next_card` 切到
  `pipeline_year_replay_disposition_decision_card`。下一步只允许继续 Pipeline disposition decision；
  这仍不是 replay rerun、System full build、Pipeline semantic repair、full rebuild、daily incremental 或 `v1 complete` 授权。
