...
incremental chain proof 也已闭环，当前 live `current_allowed_next_card` 已切到
...
- `pipeline-full-daily-incremental-chain-build-card` 已真实执行并完成：Pipeline 已按 Data -> MALF -> Alpha -> Signal -> Position -> Portfolio Plan -> Trade -> System Readout 串联 day-only daily incremental 样板，但未执行 formal release closeout
- 当前唯一 live next card 只允许 `full-rebuild-and-daily-incremental-release-closeout-card`，不得跳成 formal full rebuild passed、daily incremental release passed、Pipeline semantic repair 或 `v1 complete`
...
| 优先级 | 候选卡 | 说明 |
|---|---|---|
| P0 | `full-rebuild-and-daily-incremental-release-closeout-card` | Pipeline full daily incremental chain proof 已通过后，当前唯一 live next card |
...
| 层级 | 作用 | 当前内容 |
|---|---|---|
| 层 1 | 当前 live next card | `full-rebuild-and-daily-incremental-release-closeout-card` |
...
| 8 | `full-rebuild-and-daily-incremental-release-closeout-card` | 当前 live next：运行 full rebuild proof、daily incremental proof、resume/idempotence proof，并生成 final release evidence |
