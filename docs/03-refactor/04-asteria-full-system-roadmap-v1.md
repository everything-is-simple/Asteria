...
也已完成 truthful closeout 裁决，当前 live `current_allowed_next_card`
已切到 `system_wide_daily_dirty_scope_protocol_card`。
...
- 当前唯一 live next card 只允许 Pipeline disposition decision，不得扩成 replay rerun、System full build、Pipeline semantic repair、full rebuild、daily incremental 或 `v1 complete`
...
| 优先级 | 候选卡 | 说明 |
|---|---|---|
| P0 | `system-wide-daily-dirty-scope-protocol-card` | disposition decision 已完成 truthful closeout 后，当前唯一 live next card |
...
| 层级 | 作用 | 当前内容 |
|---|---|---|
| 层 1 | 当前 live next card | `system-wide-daily-dirty-scope-protocol-card` |
...
- 当前 live `current_allowed_next_card` 是 `system_wide_daily_dirty_scope_protocol_card`；Pipeline 已通过
  ...
  `system_wide_daily_dirty_scope_protocol_card`。下一步只允许进入 Stage 11 入口协议卡；这仍不是 replay rerun、
  System full build、Pipeline semantic repair、full rebuild、daily incremental 或 `v1 complete` 授权。
