...
### 5.1 Historical Evidence Anchors

| source | observed anchor | next-card use |
|---|---|---|
| `G:\malf-history\MarketLifespan-Quant\src\mlq\alpha\pas\contracts.py` | `BOF / BPB / PB / TST / CPB`、trigger ledger、formal signal、16-cell readout summary | PAS 触发层、formal candidate 层、历史统计读出层 |
| `G:\malf-history\MarketLifespan-Quant\src\mlq\core\contracts.py` | PAS 五触发器公共词汇与 reason code builder | 触发词汇规范化参考 |
| `G:\malf-history\MarketLifespan-Quant\docs\04-reference\battle-tested-lessons-all-modules-and-mainline-bridging-20260408.md` | trigger detection、formal signal、position/trade/system 边界经验 | 分层边界和 anti-migration guard |
| `G:\malf-history\EmotionQuant-gamma\WARP.md` | T+1 Open、IRS ranking、MSS sidecar、BOF-only 主线经验 | T+1 execution hint 与 ranking sidecar 边界 |
| `G:\malf-history\astock_lifespan-alpha\docs\02-spec\03-alpha-pas-trigger-semantic-spec-v1-20260419.md` | 早期 Alpha/PAS trigger semantic spec | 早期 trigger 语义参考，需重新映射 |
| `G:\malf-history\lifespan-0.01` | data / trade runner、checkpoint、T+1 bridge tests | 工程纪律参考，不进入 PAS 业务定义 |

阶段性裁决：

```text
listed materials are sufficient to define an independent Asteria Alpha/PAS semantics package.
listed materials are not sufficient to migrate legacy code directly.
listed materials are not sufficient to claim profitability or broker readiness.
```
...
| source sufficiency | 必须裁决 `sufficient_for_definition`，同时禁止推导为 legacy migration / profit proof |
| Alpha/PAS design set shape | 必须按 MALF v1.4 的治理形态定义 `Alpha_PAS_Design_Set_v1_0` 文件清单 |
| PAS semantic layer | 必须定义 context / trigger / strength / in-flight / historical-rank / formal-candidate 六层 |
...
| PAS trigger event | 必须定义 TST / BOF / BPB / PB / CPB 的触发、未触发、取消、失败、等待 |
| PAS historical rank profile | 必须定义同类 setup 的频率、样本量、分位、forward readout、失败/取消排名 |
