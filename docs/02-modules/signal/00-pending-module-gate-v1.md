...
Signal 是 Alpha 之后的主线模块。本目录已补齐 pre-gate 六件套 draft，但本轮不冻结 Signal 设计，不允许进入施工。

截至 2026-04-29，当前唯一允许推进的是 `Alpha freeze review`。Signal 仍必须等待
Alpha release，不得把 MALF day proof passed 解释为 Signal 施工许可。
...
Signal 进入设计冻结前必须在 Alpha released 之后重新审阅这些文档。
