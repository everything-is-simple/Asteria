...
补齐 Position pre-gate 六件套 draft，为 Signal released 之后的 Position 设计冻结做准备。

本卡不允许代码施工，不允许创建正式 DuckDB，不允许冻结 Position。
...
Position 进入 design freeze 前必须等待：
...
| 把 Signal pre-gate draft 直接当作 released 上游 | 禁止 |

## 6. 下一步入口

Signal released 后，Position 才能进入：

```text
Position design freeze review
```
