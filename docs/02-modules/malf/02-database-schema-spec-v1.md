...
| 表 | 自然键 | 说明 |
|---|---|---|
| `malf_core_run` | `run_id` | Core 构建审计与运行策略记账 |
| `malf_schema_version` | `schema_version` | schema 版本 |
| `malf_pivot_ledger` | `symbol + timeframe + pivot_dt + pivot_type + pivot_seq_in_bar + core_rule_version` | pivot 事实与 pivot detection 规则记账 |
...
| `malf_core_state_snapshot` | `symbol + timeframe + bar_dt + run_id` | Core 当前状态读取面 |

Core run / pivot / snapshot 必须带：
...
| `malf_candidate_ledger` | `candidate_event_type` | candidate_created / same_direction_candidate_refresh / opposite_direction_candidate_replacement / confirmed |
| `malf_core_state_snapshot` | `system_state` | `up_alive / down_alive / transition` 当前状态面 |
| `malf_core_state_snapshot` | `progress_updated` | bar 级 progress 更新标记 |
| `malf_core_state_snapshot` | `transition_span` | transition 期 bar 计数 |
| `malf_core_state_snapshot` | `transition_boundary_high / transition_boundary_low` | transition 当前边界 |
| `malf_core_state_snapshot` | `active_candidate_guard_pivot_id` | 当前 active candidate guard |
...
| `transition_boundary_high` | v1.4 runtime sync 字段 |
| `transition_boundary_low` | v1.4 runtime sync 字段 |
| `active_candidate_guard_pivot_id` | v1.4 runtime sync 字段 |
| `confirmation_pivot_id` | v1.4 runtime sync 字段 |
| `new_wave_id` | v1.4 runtime sync 字段 |
| `birth_type` | v1.4 runtime sync 字段 |
| `candidate_wait_span` | v1.4 runtime sync 字段 |
| `candidate_replacement_count` | v1.4 runtime sync 字段 |
| `confirmation_distance_abs` | v1.4 runtime sync 字段 |
| `confirmation_distance_pct` | v1.4 runtime sync 字段 |
| `sample_scope` | 必填 |
| `sample_version` | 必填 |
