...
| 1 | MALF | `docs/02-modules/malf/` | frozen / v1.3 day formal-data closeout passed / v1.4 authority synced / v1.4 day runtime sync code passed | 是，仅 day formal rebuild closeout | day runtime evidence 仍为 v1.3；v1.4 formal rebuild、week/month 或 full build 另需新卡 |
...
| 4 | Position | `docs/02-modules/position/` | pre-gate six-doc draft | 否，暂停 | 等 MALF v1.4 day formal rebuild closeout 后再决定是否恢复 review-only |
...
```text
MALF v1.4 Core runtime sync code card（已通过）
```

当前只允许施工对象：

```text
MALF v1.4 Core formal rebuild / runtime proof closeout
```
...
MALF v1.4 Core formal rebuild / runtime proof closeout，不允许 Position 施工。
