...
Alpha 设计冻结已由 `alpha-freeze-review-20260429-01` 通过。Alpha 施工仍必须等待：
...
MALF day release evidence 和 Alpha freeze review 均已形成 passed 结论；本文件
由 pre-gate draft 升级为 frozen authority design。该冻结不等于 Alpha 代码施工许可，
不创建正式 Alpha DB，也不打开下游模块施工。
...
这些 DB 只能在后续 Alpha bounded proof build card 明确授权后创建。
...
Alpha 已通过 freeze review。后续 bounded proof / release gate 必须满足：

| 门禁 | 要求 |
|---|---|
| MALF Release | WavePosition service released，MALF day proof passed |
| Design | Alpha 六件套已由 `alpha-freeze-review-20260429-01` 冻结 |
| Schema | 五个 alpha family DB 表族、自然键、版本字段按本规格执行 |
| Runner | bounded / segmented / full / resume 语义按本规格执行 |
| Audit | 只读 MALF、无持仓资金订单输出、自然键唯一等硬审计必须通过 |
| Evidence | Alpha bounded proof 证据落入 `H:\Asteria-report` 或 `H:\Asteria-Validated` |

当前唯一允许推进的是 `Alpha bounded proof build card`。该下一卡必须继续禁止
下游写回 MALF，并且在卡打开前不得创建正式 Alpha DB、迁移旧 Alpha engine 或运行下游
