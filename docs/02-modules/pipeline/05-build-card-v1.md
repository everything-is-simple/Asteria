...
`coverage-gap-evidence-incomplete-closeout-card-20260509-01`。Pipeline 当前只保留 blocked replay truth、
Alpha/Signal repair passed truth 与 downstream gap 归因，不再拥有新的 live repair 施工位；后续如需继续推进
System Readout repair、Pipeline source-selection repair、full rebuild、daily incremental 或 `v1 complete`，
都必须等 downstream closeout 形成结论后再单独授权。
