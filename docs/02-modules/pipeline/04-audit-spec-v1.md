...
Pipeline 审计用于证明 Pipeline 只做编排和记录，不定义业务语义，不回写任何业务模块，并且遵守当前已释放的单模块 / full-chain dry-run 施工门禁。
...
| 试图使用 `full / segmented / daily_incremental`，或把 `bounded` 用在 `full_chain_day` | hard fail |
...
当前 released 证据已证明 `single-module orchestration build passed` 与 `full-chain dry-run passed`。
任何把当前 dry-run 账本冒充为 full-chain bounded proof，或绕过新卡直接扩成 bounded proof 的行为，都应直接计为 hard fail。
