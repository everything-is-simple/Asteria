...
当前 live `current_allowed_next_card` 是 `portfolio_plan_2024_coverage_repair_card`。
...
语义断点已下移到 Portfolio Plan。因此当前唯一 live repair 施工位已经收口到 Portfolio Plan，不允许直接跳去
Trade repair、System Readout repair、Pipeline source-selection repair、full rebuild、daily incremental
或 `v1 complete`。
