...
| current next card | `alpha_signal_daily_incremental_ledger_build_card` |
...
当前 live `current_allowed_next_card` 是 `alpha_signal_daily_incremental_ledger_build_card`。
...
`malf-daily-incremental-ledger-build-card` 已依次完成 `day`-only 样板推进。
因此当前唯一 live 施工位已继续前进到 Alpha/Signal daily hardening；在该卡完成之前，
仍不得直接跳去下游 daily runtime、full rebuild、System full build、Pipeline semantic repair 或 `v1 complete`。
