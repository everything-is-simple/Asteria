...
可证事实优先口径完成最小正式化，并已部分释放可匹配正式 Data 标的的申万 2021
当前行业快照。`data-foundation-production-baseline-seal-20260502-01` 将 Data
正式封为主线输入底座；后续 Data 只能通过明确 maintenance card 扩展。
...
交易日历、标的主数据、源代码别名、观测宇宙和执行价线可交易事实。
`data-market-meta-sw-industry-snapshot-20260502-01` 进一步放行申万 2021 当前行业
快照中可匹配正式 Data 标的的 4,237 行。index/block、ST、停牌、真实上市/退市与
历史行业沿革仍需另开卡。
`data-foundation-production-baseline-seal-20260502-01` 不重建任何 DB，只确认当前
五个正式 Data DB 是主线输入底座封版。
...
当前 `market_meta.duckdb` 的申万 2021 当前行业快照已部分释放；ST、停牌、真实
上市/退市状态与历史行业沿革仍为 source gap。这些缺口必须保留为后续
reference-source expansion 卡，不得在本卡中伪造。

## 13. 上线门禁

Data Foundation 当前生产级地基已通过 `data-foundation-production-baseline-seal-20260502-01`
封为主线输入底座。后续扩展只能以 maintenance card 进入，且仍必须满足：
