...
截至 2026-05-02，Data Foundation 已冻结旧版 Lifespan 本地 raw/base 库的最小导入合同；
完整 Data Foundation 设计、meta build、全量 runner 和 full release 仍未放行。
...
旧库导入合同只允许创建或 promote `raw_market.duckdb` 与
`market_base_day/week/month.duckdb` 的 `stock / backward` 首轮正式版本。
`market_meta.duckdb`、index/block 主线接入与完整 Data full build 仍需另开卡。
...
| `market_meta.duckdb` | `trade_calendar`, `instrument_master`, `industry_classification`, `universe_membership`, `tradability_fact`, `meta_run`, `meta_schema_version` |
...
当前打开的是 Data legacy source intake 五卡链路。该链路只补地基输入，不打开
Position 或其他下游施工，也不得被当作策略模块施工位。
