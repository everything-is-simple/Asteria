...
| 当前允许下一卡 | data reference target maintenance scope |
...
| 目标 | 状态 |
|---|---|
| Data Reference Scope | passed，已冻结 ST、停牌、上市/退市、历史行业、index/block、week/month execution line 的 closeout 范围 |
| Data Reference Closeout | prepared，按 scope card 补 Data reference facts、DB 表面和审计 |
| Repair Queue | 已拆为 7 张卡，按 Data -> MALF -> Alpha -> Signal -> 总控裁决执行 |
| Position Build | Position bounded proof build card 已准备但暂停施工，等待上游 release decision |
...
```text
MALF week/month 放行、Alpha/Signal production build、Position bounded proof、Position 及下游主线实现、pipeline runtime 或任何未经放行证据支撑的正式主线运行。
```

这正是本轮重构想要的状态：先把地基、边界和证据摆正，再按七张上游修补卡逐张打开后续 bounded proof / production hardening。
