...
-> data reference target maintenance scope 已通过 -> data reference target maintenance closeout 已通过 -> MALF week bounded proof build 已通过 -> MALF month bounded proof build 已通过 -> Alpha production builder hardening 已通过 -> Signal production builder hardening
```
...
和 Alpha production builder hardening 已通过，当前只授权 Signal production builder hardening；Position bounded proof
施工已暂停，直到 Data / MALF / Alpha / Signal 的修补卡序完成并由总控 release decision 裁定。不授权
Signal full build beyond this card、Position full build、下游施工或全链路 pipeline。
