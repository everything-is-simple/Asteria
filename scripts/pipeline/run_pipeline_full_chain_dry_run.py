...
from asteria.pipeline.bootstrap import run_pipeline_full_chain_dry_run


def main() -> int:
    parser = argparse.ArgumentParser(description="Run Pipeline full-chain dry-run.")
    ...
    parser.add_argument("--mode", choices=["dry-run", "resume"], default="dry-run")
    parser.add_argument("--run-id", required=True)
    parser.add_argument("--source-chain-release-version", required=True)
    args = parser.parse_args()
    summary = run_pipeline_full_chain_dry_run(
